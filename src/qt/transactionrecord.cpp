        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
				...
				if(nHeight <= 1 && nHeight > 0) {
					nSubsidy = 30000 * COIN;
					if(nSubsidy * 0.9 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 1 && nHeight <= 181) {
					nSubsidy = 0.1 * COIN;
					if(nSubsidy * 0.9 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 181 && nHeight <= 4502) {
					nSubsidy = 0.2 * COIN;
					if(nSubsidy * 0.9 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 4502 && nHeight <= 76503) {
					nSubsidy = 20 * COIN;
					if(nSubsidy * 0.9 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}

				} else if (nHeight > 76503 && nHeight <= 522904) {
					nSubsidy = 1.5 * COIN;
					if(nSubsidy * 0.9 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}

				} else if (nHeight > 522904) {
					nSubsidy = 0.18 * COIN;
					if(nSubsidy * 0.9 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
