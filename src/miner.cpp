...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LUQMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("LUQMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("LUQMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("luq-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("LUQMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running LUQMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadLUQMiner() exception");
    } catch (...) {
        LogPrintf("ThreadLUQMiner() exception");
    }

    LogPrintf("ThreadLUQMiner exiting\n");
}
